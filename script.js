/*
  ...
  Non-obvious decisions: all form submissions are local demo actions so the
  site works on GitHub Pages, and event calendar files are generated in-browser.
*/
...
document.querySelectorAll("form[data-form]").forEach((form) => {
  form.addEventListener("submit", (event) => {
    ...
    if (status) {
      if (kind === "donation") {
        status.textContent = `Pledge noted for $${data.amount}. Add a real processor before collecting money.`;
      } else if (kind === "event-rsvp") {
        status.textContent = "RSVP saved in this browser. Bring the friend if you checked the box.";
      } else {
        status.textContent = "You are on the list. The campaign room just got less empty.";
      }
    }
